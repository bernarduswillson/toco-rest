...
router.post("/result/:exercise_id", async (req, res) => {
  ...
  try {
    ...
    if (result !== null) {
      result.questions.forEach((question) => {
        const selectedOption =
          selectedOptions[`question_${question.question_id}`]
        if (selectedOption !== undefined) {
          const isCorrect = question.options.find(
            (opt) =>
              opt.option_id === parseInt(selectedOption) && opt.is_correct
          )
          ...
          wrongResults[question.question_id] = correctAnswer
            ? correctAnswer.option_id.toString()
            : null
        }
      })
    }
    ...
    if (startIndex !== -1 && endIndex !== -1) {
      const returnValue = soapXml.substring(
